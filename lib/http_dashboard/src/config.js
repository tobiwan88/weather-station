...
var TOKEN_KEY = 'http_dashboard_token';

function getToken() { return localStorage.getItem(TOKEN_KEY) || ''; }
function saveToken(t) { localStorage.setItem(TOKEN_KEY, t); }

function showTokenCard(highlight) {
  var card = document.getElementById('tokenCard');
  if (!card) return;
  card.scrollIntoView({ behavior: 'smooth' });
  if (highlight) { card.classList.add('card-error'); }
}
...
function onTokenSave() {
  ...
  if (status) {
    status.textContent = t ? 'Token saved.' : 'Token cleared.';
...
function postForm(form) {
  ...
  return fetch('/api/config', {
    ...
    if (r.status === 401) {
      ...
      if (inp) inp.value = '';
      showTokenCard(true);
      throw new Error('Unauthorized — enter your bearer token and try again.');
    }
...
// Pre-fill token input from localStorage.
(function () {
  var inp = document.getElementById('tokenInput');
