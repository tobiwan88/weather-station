...
const char *mqtt_publisher_type_to_topic_str(enum sensor_type t)
{
	...
	case SENSOR_TYPE_UV_INDEX:    return "uv_index";
	case SENSOR_TYPE_BATTERY_MV:  return "battery_mv";
...
{
	switch (t) {
	case SENSOR_TYPE_TEMPERATURE: return "\xc2\xb0\x43"; /* °C (UTF-8) */
	case SENSOR_TYPE_HUMIDITY:    return "%";
	case SENSOR_TYPE_PRESSURE:    return "hPa";
	case SENSOR_TYPE_CO2:         return "ppm";
	case SENSOR_TYPE_LIGHT:       return "lux";
	case SENSOR_TYPE_UV_INDEX:    return "";
	case SENSOR_TYPE_BATTERY_MV:  return "mV";
	default:                      return "";
	}
}
