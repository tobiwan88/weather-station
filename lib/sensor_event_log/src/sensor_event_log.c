...
static void sensor_event_log_cb(const struct zbus_channel *chan)
{
	...
	case SENSOR_TYPE_TEMPERATURE:
		LOG_INF("[%02d:%02d:%02d.%03d]  0x%04x  TEMP  %7.2f °C",
			hh, mm, ss, ms, evt->sensor_uid,
			q31_to_temperature_c(evt->q31_value));
		break;
	case SENSOR_TYPE_HUMIDITY:
		LOG_INF("[%02d:%02d:%02d.%03d]  0x%04x  HUM   %6.1f %%RH",
			hh, mm, ss, ms, evt->sensor_uid,
			q31_to_humidity_pct(evt->q31_value));
		break;
	default:
		LOG_INF("[%02d:%02d:%02d.%03d]  0x%04x  type=%-3d  q31=0x%08x",
			hh, mm, ss, ms, evt->sensor_uid,
			evt->type, (uint32_t)evt->q31_value);
		break;
	}
}
...
{
	return zbus_chan_add_obs(&sensor_event_chan, &sensor_event_log_listener,
				 K_NO_WAIT);
}
